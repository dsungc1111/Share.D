...
### **문제**: 로그인 시, 액세스 토큰 저장 방법 `UserDefault.accessToken = "token"`,   
토큰 갱신 시, 액세스 토큰 저장 방법(Key값은 동일) `UserDefaults.standard.setValue(token, forKey: "newToken")`, Key값만 같다면 값이 갱신될 것이라고 생각했지만, 갱신이 되지 않아 무한 리콜 발생   
...
### - 네트워크 로직 구성에 대한 고민
  -  유형별(유저, 토큰, 포스트 관련)로 라우터와 메서드를 구조화하면 보다 직관적이고 효율적인 메서드 호출이 가능하다고 생각해, 유형에 따라  네트워크 로직 구현
- 하지만 활용되는 API의 유형이 훨씬 많아진다면? 
- 제네릭을 이용한 이유는 많은 API를 핸들링하는 과정에서 데이터를 유동적으로 받아오고 중복되는 코드를 줄이기 위함
- 하지만 현재 코드는 제네릭을 사용하면서도 유형별로 라우터와 메서드를 나누어 놓았기 때문에 `중복되는 코드 발생, API수가 많아지면 더 많은 중복 코드가 발생` 
- 고민해야할 부분   
  1. 중복되는 메서드를 줄이려면?  
  2. 메서드를 줄였다 쳐도, Router는 여전히 유형별로 나눠 사용할지 하나로 합쳐 사용할지?
